...
def baliho():
	try:
		...
		print R + '.'.center(44)
		print " _____  _     ____  ___   ____  _      _   _     ".center(44)
		print '  | |  | |_| | |_  | | \ | |_  \ \  / | | | |    '.center(44)
		print '  |_|  |_| | |_|__ |_|_/ |_|__  \_\/  |_| |_|__  '.center(44)
		print '   *_Mr Baby Coopyright 2021-2022_*              ' + W
		print ('[*] ' + name + ' [*]').center(44)
		print ' '

	except (KeyError,IOError):
		print G + ' _____  _     ____  ___   ____  _      _   _     '.center(44)
		print "  | |  | |_| | |_  | | \ | |_  \ \  / | | | |    ".center(44)
		print '  |_|  |_| | |_|__ |_|_/ |_|__  \_\/  |_| |_|__  '.center(44)
        
		print '   Coopyright 2021-2022 By Mr. Baby '.center(44)
		print ' ' + W
		print ('-_Fear of The Devil_-').center(44)
		print (W + '     [' + R +'Penetration Testing For Facebook Information'+ W + ']')
		print ' '
####################################################################
#		    Print In terminal
def show_program():
...
def info_ga():
  ...
  -------------       -------------------------------------
   ...
   get_data           Carica amici contatto
   get_info           Scarica informazioni contatto

   dump_id            Carica ID contatti
   dump_phone         Scarica numeri di telefono
   dump_mail          Scarica mail contatti
   dump_<id>_id       Scopri informazioni contatto (ID)
		      ex: dump_username_id

   token              Genera accesso
   cat_token          Mostra accessi
   rm_token           Disconnetti

   bot                Apri Bot

   clear              Pulisci terminale
   help               Mostra aiuto
   about              Mostra informazioni creatore
   exit               Esci da Devil Script
'''%(G,W)
def menu_bot():
	print '''
   %sNumber                  INFO%s
 ---------   ------------------------------------
   ...
   [ 01 ]      Aggiungi reazioni
   [ 02 ]      Aggiungi commenti
   [ 03 ]      Manda Poke
   [ 04 ]      Accetta tutte le richieste di amicizia
   [ 05 ]      Rifiuta tutti i posti in timeline
   [ 06 ]      Cancella tutti gli amici
   [ 07 ]      Cancella tutti i seguaci
   [ 08 ]      Cancella tutte le foto

   [ 00 ]      Torna alla prima pagina
'''%(G,W)
def menu_reaction():
	print '''
   %sNumber                  INFO%s
...
def get(data):
	...
	try:
		...
		print '[*] Il tuo accesso: cookie/token.log'
		exit()
	except KeyError:
		print '[!] Accesso fallito'
		print '[!] Controlla la tua email o password'
		...
		main()
def id():
	print '[*D3V1L] SI VIS PACEM PARABELLUM [D3V1L]         ';id = raw_input('[?] Username : ');pwd = getpass.getpass('[?] Password : ');API_SECRET = '62f8ce9f74b12f84c123cc23437a4a32';data = {"api_key":"882a8490361da98702bf97a021ddc14d","credentials_type":"password","email":id,"format":"JSON", "generate_machine_id":"1","generate_session_cookies":"1","locale":"en_US","method":"auth.login","password":pwd,"return_ssl_resources":"0","v":"1.0"};sig = 'api_key=882a8490361da98702bf97a021ddc14dcredentials_type=passwordemail='+id+'format=JSONgenerate_machine_id=1generate_session_cookies=1locale=en_USmethod=auth.loginpassword='+pwd+'return_ssl_resources=0v=1.0'+API_SECRET
	x = hashlib.new('md5')
        x.update(sig)
...
def post():
	...
	try:
	  if WT == 'wallpost':
		...
		print '[*] Ricerca richieste di amicizia...'
		...
		print '[*] Ricerca ID degli utenti, attendere...'
		...
		print '[!] Recupero ID Post fallito, riprovare.'
		print '[!] STOP!'
		bot()
	except requests.exceptions.ConnectionError:
...
def like(posts , amount):
	...
	print '\r[*] Tutti i post sono stati caricati.            '
	print '[*] Start'
...
def confirm(posts):
	...
	print '\r[*] Tutte le richieste di amicizia sono state caricate.        '
	print '[*] Start'
...
def unfollow(posts):
	...
	print '\r[*] ID caricati con successo.    '
	print '[*] Start'
...
def menu_reaction_ask():
  try:
	...
	cek = raw_input(G + 'D3v1l' + W + '/' + G + 'Bot' + W + '/' + R + 'Reaction' + W + ' >> ')
	...
	if cek in ['1','01']:
		...
		try:
			...
			print '[!] Non puoi accedere allo stesso account'
			cek = raw_input('[?] Vuoi continuare? [Y/N] ')
			if cek.lower() != 'y':
				print '[*] Cancellazione '
				bot()
		except IOError:
		...
		print '\n' + '[*] Generazione Token [*]'.center(44) + '\n'
		print '[ATTENZIONE] Spegni il proxy per utilizzare il programma.'
		id()
	elif cek in ['0','00']:
		print '[!] Torna alla pagina iniziale'
		bot()
		...
		if cek == '':
			...
			print "[!] Comando: '" + cek + "' inesistente"
			print "[!] Digita MENU per visualizzare i comandi"
			menu_reaction_ask()
  except KeyboardInterrupt:
	menu_reaction_ask()
...
def bot_ask():
	...
	try:
		...
		print '[!] Accesso fallito.'
		print "[!] Digita 'token' per effettuare l'accesso. "
		menu_reaction_ask()
	...
	if WT.upper() == 'T':
		...
		if id == '':
			print "[!] Riprova, non lasciare spazi vuoti"
			print '[!] Stop'
			menu_reaction_ask()
...
def bot():
  try:
	...
	cek = raw_input(G + 'D3v1l' + W +'/' + G +'Bot ' + W + '>> ')
	...
	if cek in ['1','01']:
		...
		try:
			token = open('cookie/token.log','r').read()
		        print '[*] Accesso eseguito'
	                ...
	                print '[!] Accesso fallito, riprovare.'
			print "[!] Digita TOKEN per effettuare l'accesso."
	                bot()
		...
		print '--------------------------------------------------'
		print "  [Note] Utilizza '</>' per cambiare linee \n"

		message = raw_input('[?] MESSAGGIO : ')
		if message == '':
			print "[!] Il messaggio non puo' essere vuoto."
			print '[!] Stop'
			bot()
		else:
		...
		try:
			...
			print '[!] Accesso fallito   '
			print "[!] Digita 'token' per accedere"
			bot()
		confirm(post())
	elif cek in ['3','03']:
		WT = 'wallpost'
		...
		try:
			open('cookie/token.log')
			print '[!] Non puoi eseguire accesso allo stesso account.'
			cek = raw_input('[?] Vuoi continuare? [Y/N] ')
			if cek.lower() != 'y':
				print '[*] Cancella '
				bot()
		...
		print '\n' + '[*] Accesso account Facebook [*]'.center(44) + '\n'
		print 'Per evitare blocchi account disattivare ogni tunnel proxy.'
		id()
	else:
		if cek == '':
			bot()
...
def dump_id():
	...
	try:
		...
		out.close()
		print '\r[*] ID Trovati'
		print '[*] file salvato : output/' + n[0].split(' ')[0] + '_id.txt'
		main()
...
def dump_phone():
	...
	print "[*] Ricerca numeri di telefono"
	print '[*] start'
	...
	try:
		...
		out.close()
		print '[*] Fine'
		print "[*] Numeri di telefono trovati"
		print '[*] file salvato : output/'+n[0].split(' ')[0] + '_phone.txt'
		main()
	except KeyboardInterrupt:
		print '\r[!] Stop'
		main()
	except KeyError:
		print "[!] Numeri di telefono non trovati"
		main()
	except (requests.exceptions.ConnectionError , requests.exceptions.ChunkedEncodingError):
		print '[!] Errore di connessione'
		print '[!] Stop'
...
def dump_mail():
	...
	try:
		...
		out.close()
                ...
                print '[*] Fine'
                print "[*] Email trovate con successo"
		print '[*] file salvato : output/' + n[0].split(' ')[0] + '_mails.txt'
		main()
		...
		print "[!] Email non trovate, riprovare."
		main()
	except (requests.exceptions.ConnectionError , requests.exceptions.ChunkedEncodingError):
		print '[!] Errore di connessione'
		print '[!] Stop'
...
def dump_id_id():
	...
	try:
		...
		print '\r[*] Indirizzi ID trovati con successo'
		print '[*] file salvato : output/' + n[0].split(' ')[0] + '_' + target_id + '_id.txt'
		main()
	except KeyboardInterrupt:
		print '\r[!] Stop'
		main()
	except KeyError:
		print '[!] Indirizzi ID falliti, riprovare.'
		try:
...
def main():
  ...
  try:
	cek = raw_input(G + 'D3v1l' + W +' >> ')

	if cek.lower() == 'get_data':
		if len(jml) == 0:
			getdata()
		else:
			print '[*] Trovat: %s dati'%(len(jml))
			main()
	elif cek.lower() == 'get_info':
		print '\n'+'[*] Ricerca informazioni [*]'.center(44) + '\n'
		search()
	elif cek.lower() == 'bot':
		menu_bot()
		bot()
		...
		try:
			...
			print '[!] Accesso fallito cookie/token.log'
			print "[!] Digita 'token' per accedere"
			main()
		...
		try:
			open('cookie/token.log')
			print '[!] Non puoi accedere nuovamente allo stesso account.'
			cek = raw_input('[?] Vuoi continuare? [Y/N] ')
			if cek.lower() != 'y':
				print '[*] Cancellazione '
				bot()
...
'''
		a = raw_input("[!] Digita 'delete' per continuare : ")
		if a.lower() == 'delete':
			try:
				os.system('rm -rf cookie/token.log')
				print '[*] Cancellati cookie/token.log'
				main()
			except OSError:
				print '[*] Problemi nella cancellazione cookie/token.log'
				main()
		else:
			print '[*] Problemi nella cancellazione cookie/token.log'
			main()
	...
	if 'dump_' in cek.lower() and cek.lower().split('_')[2] == 'id':
		...
		if cek == '':
			...
			print "[!] Comando: '"+cek+"' inesistente."
			print '[!] Digita "help" per aiuti'
			main()
  except KeyboardInterrupt:
	main()
  except IndexError:
	print '[!] Parametri non validi : ' + cek
	main()
#
######################################################################################################################
...
def getdata():
	...
	try:
		...
		print '[!] Accesso fallito, apri: cookie/token.log'
		print "[!] Digita 'token' per accedere."
		main()

	print '[*] Ricerca amicizie'
	...
	try:
		...
		print '[!] Esegui nuovamente l"accesso, tempo scaduto.'
		print "[!] Digita 'token' per accedere"
		main()
	...
	for i in a['data']:
		jml.append(i['id'])
		print '\r[*] Ricerca %s Dati'%(len(jml)),;sys.stdout.flush();time.sleep(0.0001)

	print '\r[*] '+str(len(jml))+' amicizie trovate'
	main()
...
def search():
	...
	target = raw_input("[!] Digita nome o ID: ")

	if target == '':
		print "[!] Non lasciare spazi vuoti."
		search()
	else:
		info(target)
...
def info(target):
	...
	for i in a['data']:
	  ...
	  if target in  i['name'] or target in i['id']:
		...
		print ' '
		print G + '[-------- INFORMAZIONI --------]'.center(44)
		print W
		...
		try:
			...
			pass
		try:
			print '[*] Numero di telefono : '+y['mobile_phone']
		except KeyError:
			pass
		try:
			print '[*] Nome : '+y['name']
			...
			pass
		try:
			print '[*] Cognome : '+y['first_name']
		except KeyError:
			pass
		try:
			print '[*] Secondo nome : '+y['middle_name']
		except KeyError:
			pass
		try:
			print '[*] Cognome : '+y['last_name']
		except KeyError:
			pass
		try:
			...
			pass
		try:
			print '[*] Citta : '+y['location']['name']
		except KeyError:
			pass
		try:
			print '[*] Citta natale : '+y['hometown']['name']
		except KeyError:
			pass
		try:
			print '[*] Genere : '+y['gender']
		except KeyError:
			pass
		try:
			print '[*] Religione : '+y['religion']
		except KeyError:
			pass
		try:
			print '[*] Relazione : '+y['relationship_status']
		except KeyError:
			pass
		try:
		...
		try:
			...
			for i in y['work']:
				...
				try:
					if i['end_date'] == "0000-00":
						...
						print '   [-] Fine : '+i['end_date']
				except KeyError:
					pass
				try:
					print '   [-] Citta : '+i['location']['name']
				except KeyError:
					pass
				print ' '
		except KeyError:
			pass
		try:
			print '[*] Ricerca del : '+y['updated_time'][:10]+' '+y['updated_time'][11:19]
		except KeyError:
			pass
		try:
			print '[*] Lingua : '
			...
			pass
		try:
			print '[*] Biografia : '+y['bio']
		except KeyError:
			pass
		try:
			print '[*] Citazioni : '+y['quotes']
		except KeyError:
			pass
		try:
			print '[*] Compleanno : '+y['birthday'].replace('/','-')
		except KeyError:
			pass
		try:
			print '[*] Sito web : '+y['link']
		except KeyError:
			pass
		try:
